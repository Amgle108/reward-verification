...
if run_mode == 'slc_prizes':
    ...
    raffle_seed = 'Silencio_Raffle_Season1:Rewards54%-Decentralized30%-Sustainability11%-Security5%-SLC:'
elif run_mode == 'physical_prizes':
    input_prize_matrix = 'token_assignment/physical_prizes.csv'
    raffle_seed = 'Silencio_Raffle_Season1:Rewards54%-Decentralized30%-Sustainability11%-Security5%-Physical:'

prize_assignment = pd.read_csv(input_prize_matrix)
total_prize_quantity = prize_assignment['prize_quantity'].sum()
# get user data
user_data = pd.read_csv('silencio_raffle_eligible_users_march_2025.csv', dtype={'raffle_tickets': 'int64'})
...
results_by_user = user_data[['hashed_user_id', 'raffle_tickets']].rename(columns={'raffle_tickets':'tickets'}).merge(raffle[['hashed_user_id', 'claimableAmount']], on='hashed_user_id', how='inner')\
    .merge(raffle_user_prizes_values.pivot_table(index = 'hashed_user_id', columns='prize_name', values='quantity', fill_value = 0)
             , on='hashed_user_id', how='left').fillna(0).sort_values('claimableAmount', ascending=False)[['hashed_user_id', 'tickets', 'claimableAmount'] + prize_assignment['prize_name'].values.tolist()].to_csv(f'raffle_results/{run_mode}_raffle_results.csv', index=False)
